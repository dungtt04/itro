...
?>
<div class="container">
    <h2>Sửa chỉ số nước</h2>
    <?php if (!empty($msg)): ?><div class="msg"><?= $msg ?></div><?php endif; ?>
    <form method="post">
        <label>Tháng (YYYY-MM)</label>
        <input type="text" name="month" id="month" value="<?= htmlspecialchars($w['month']) ?>" required oninput="updateObjectName();">
        <label>Phòng</label>
        <select name="room_id" id="room_id" required onchange="updateObjectName();">
            <option value="">-- Chọn phòng --</option>
            <?php foreach($rooms as $r): ?>
                <option value="<?= $r['id'] ?>" <?= $w['room_id']==$r['id']?'selected':'' ?>><?= htmlspecialchars($r['room_code']) ?></option>
            <?php endforeach; ?>
        </select>
        <label>Tên đối tượng</label>
        <input type="text" name="object_name" id="object_name" value="<?= htmlspecialchars($w['object_name']) ?>" readonly required>
        <label>Chỉ số cũ (CSC)</label>
        <input type="text" name="CSC" id="CSC" value="<?= $w['CSC'] ?>" readonly>
        <label>Chỉ số mới (CSM)</label>
        <input type="text" name="CSM" id="CSM" value="<?= $w['CSM'] ?>" required oninput="calcDTT();">
        <label>Số nước tiêu thụ (DTT)</label>
        <input type="text" name="DTT" id="DTT" value="<?= $w['DTT'] ?>" readonly>
        <label>Thành tiền</label>
        <input type="text" name="total" id="total" value="<?= $w['total'] ?>" readonly>
