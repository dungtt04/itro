...
<div class="revenue-container">
    ...
    <div class="tabs">
        <div class="tab active">Theo Tháng</div>
    ...
    <div class="filter-group">
        <form method="get" action="index.php" style="display: flex; gap: 15px; align-items: flex-end;">
            ...
            <div>
                <label style="display: block; font-size: 12px; color: #666; margin-bottom: 5px;">Tháng</label>
    ...
    <div class="stats-grid">
        <div class="stat-card success">
            <div class="stat-label">Doanh thu (Sau chiết khấu)</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['total_revenue'] ?? 0)) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">đ</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Doanh thu (Trước chiết khấu)</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['gross_revenue'] ?? 0)) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">đ</div>
        </div>
        <div class="stat-card info">
            <div class="stat-label">Tiền phòng</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['room_revenue'] ?? 0)) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">đ</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Tiền điện</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['electricity_revenue'] ?? 0)) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">đ</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Tiền nước</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['water_revenue'] ?? 0)) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">đ</div>
        </div>
        <div class="stat-card warning">
            <div class="stat-label">Phí dịch vụ</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['service_revenue'] ?? 0)) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">đ</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Số hóa đơn</div>
            <div class="stat-value"><?= (int)($monthlyStats['invoice_count'] ?? 0) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">cái</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Số phòng</div>
            <div class="stat-value"><?= (int)($monthlyStats['total_rooms'] ?? 0) ?></div>
            <div style="font-size: 12px; color: #666; margin-top: 5px;">phòng</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Doanh thu trung bình</div>
            <div class="stat-value"><?= number_format((int)($monthlyStats['avg_revenue'] ?? 0)) ?></div>
    ...
    <div class="table-container">
        <h3 style="padding: 20px 15px 0 15px; margin: 0 0 10px 0; color: #093d62;">Doanh thu chi tiết theo phòng</h3>
        <table>
            <thead>
                <tr>
                    <th>Phòng</th>
            ...
            <tbody>
                ...
                <tr>
                    <td><strong><?= htmlspecialchars($room['room']) ?></strong></td>
                    <td class="text-right table-mobile-hidden"><?= number_format((int)($room['room_revenue'] ?? 0)) ?> đ</td>
                    <td class="text-right table-mobile-hidden"><?= number_format((int)($room['electricity_revenue'] ?? 0)) ?> đ</td>
                    <td class="text-right table-mobile-hidden"><?= number_format((int)($room['water_revenue'] ?? 0)) ?> đ</td>
                    <td class="text-right table-mobile-hidden"><?= number_format((int)($room['service_revenue'] ?? 0)) ?> đ</td>
                    <td class="text-right"><?= number_format((int)($room['gross_revenue'] ?? 0)) ?> đ</td>
                    <td class="text-right text-success"><?= number_format((int)($room['total_revenue'] ?? 0)) ?> đ</td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <div style="background: white; padding: 40px; text-align: center; border-radius: 8px; color: #666;">
        <p>Không có dữ liệu doanh thu cho tháng <?= str_pad($month, 2, '0', STR_PAD_LEFT) ?>/<?= $year ?></p>
    </div>
    <?php endif; ?>
</div>
