...
function fetchCSC() {
    ...
    if(room_id && month && year) {
        ...
        xhr.open("GET", "index.php?controller=electricity&action=ajax_csc&room_id="+room_id+"&month="+month+"&year="+year, true);
        xhr.onload = function() {
            if(xhr.status === 200) {
...
function calcDTT() {
    ...
    // Tính thành tiền = DTT * unit_price ( lấy theo ô input unit_price hoặc mặc định nếu input để trống)
